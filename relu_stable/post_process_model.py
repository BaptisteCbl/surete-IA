...
parser.set_defaults(output="model")
...
def prune_small_weights(tf_vars, sess, tolerance):
  for tf_var in tf_vars:
    ...
    logger_pp.info("remaining non zero weights, total weights, proportion")
    logger_pp.info("{}, {}, {}".format(len(np.where(abs(weights) != 0)[0]), len(weights.flatten()), len(np.where(abs(weights) != 0)[0])/len(weights.flatten())))
...
def evaluate_checkpoint(filename, weight_prune, tolerance, relu_prune, relu_prune_frac):
  with tf.Session() as sess:
    ...
    if do_eval:
      ...
      logger_pp.info("% nat acc, % adv acc")
      logger_pp.info("{:.2f},{:.2f}".format(100 * acc_nat, 100 * acc_adv))
    ...
    if relu_prune:
      ...
      def get_ops(adv, relu_prune_frac):
          ...
          logger_pp.info("number of relus left: {} on {}".format(str(len(ops[ops==0])),str(len(ops[ops>=-1]))))
          return ops
      ...
      #if do_eval: NOT WORKING TOO MUCH MEMORY NEED
      if False:
...
print("Processing model from {}".format(model_dir+"\\ckpt"))
cur_checkpoint = tf.train.latest_checkpoint(model_dir+"\\ckpt")

log_pp = matdir + "\\log_postprocess.csv"
logger_pp = logging.getLogger("pp")
logger_pp.setLevel(logging.INFO)
for handler in logger_pp.handlers[:]:
    logger_pp.removeHandler(handler)
handler_pp = logging.FileHandler(log_pp)
logger_pp.addHandler(handler_pp)

logger_pp.info("# " + "model_dir="+str(model_dir)+", weight_prune="+str(weight_prune)+", relu_prune="+str(relu_prune)+", do_eval="+str(do_eval)+", relu_prune_frac="+str(relu_prune_frac)+", weight_thresh"+str(weight_thresh))
...
logger_pp.info("Time")
logger_pp.info("{:.2f}".format(training_time))
...
sio.savemat(matdir+'\\{}'.format(args.output), all_weights)
