...
def get_grad_np(
    model, batches, eps, opt, half_prec, scaler, rs=False, cross_entropy=True
):
    grad_list = []
    for i, (X, y) in enumerate(batches):
        ...
        if cross_entropy:
            ...
            margin = (
                preds_correct_class - logits
            )  # difference between the correct class and all other classes
            margin += (
                y_onehot.float() * 10000
            )  # to exclude zeros coming from f_correct - f_correct
            margin = margin.min(1, keepdim=True)[0]
            loss = F.relu(1 - margin).mean()

        if half_prec:
            scaler.scale(loss).backward()
            delta.grad.mul_((loss / loss).item())
        else:
            loss.backward()
...
def get_input_grad(
    model, X, y, opt, eps, half_prec, scaler, delta_init="none", backprop=False
):
    if delta_init == "none":
    ...
    if half_prec:
        ...
        grad = torch.autograd.grad(
            loss, delta, create_graph=True if backprop else False
        )[0]
        grad /= loss / ploss
    else:
        grad = torch.autograd.grad(
    ...
    for _ in range(attack_iters):
        if half_prec:
            with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                ...
                loss = F.cross_entropy(output, y)
            scaler.scale(loss).backward()
            delta.grad.mul_(loss.item() / loss.item())
    ...
    for i_restart in range(n_restarts):
        ...
        for _ in range(attack_iters):
            ...
            if not l2_grad_update:
                ...
                delta.data = (
                    delta
                    + alpha * grad / (grad**2).sum([1, 2, 3], keepdim=True) ** 0.5
                )
        ...
        with torch.no_grad():
            if half_prec:
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    all_loss = F.cross_entropy(
                        output, y, reduction="none"
                    )  # .detach()  # prevents a memory leak
            else:
                output = model(X + delta)
            ...
            if verbose:  # and n_restarts > 1:
                print(
                    "Restart #{}: best loss {:.3f}".format(i_restart, max_loss.mean())
                )
...
def rob_acc(
    ...
    for i, (X, y) in enumerate(batches):
        ...
        pgd_delta = attack_pgd(
            ...
            verbose=verbose,
            linf_proj=linf_proj,
            l2_grad_update=l2_grad_update,
            cuda=cuda,
        )
        if corner:
            pgd_delta = clamp(X + eps * sign(pgd_delta), 0, 1, cuda) - X
        pgd_delta_proj = (
            clamp(X + eps * sign(pgd_delta), 0, 1, cuda) - X
        )  # needed just for investigation
...
def get_lr_schedule(lr_schedule_type, n_epochs, lr_max):
    if lr_schedule_type == "cyclic":
        lr_schedule = lambda t: np.interp(
            [t], [0, n_epochs * 2 // 5, n_epochs], [0, lr_max, 0]
        )[0]
...
def backward(loss, opt, half_prec, scaler):
    if half_prec:
        scaler.scale(loss).backward()
    else:
        loss.backward()
